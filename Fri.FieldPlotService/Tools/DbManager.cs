...
namespace Fri.FieldPlotService.Tools
{
    ...
    public class DbManagerOptions
    {
        ...
        {
            ConnectionString = configuration["Database:ConnectionString"] ?? throw new FieldPlotServiceException(FieldPlotServiceError.ConfigurationError, "The Database:ConnectionString is missing.");
    ...
    {
        /// <summary>
        /// Format for CosmosDB Date Time
        /// </summary>
        public const string CosmosDbDateTimeFormat = "yyyy-MM-ddTHH:mm:ss.fffffffZ";
        ...
        /// <param name="containerName"></param>
        /// <returns></returns>
        public async Task DeleteAsync<T>(Guid id, string? partitionKey = null, bool forceDelete = false, string? containerName = null) where T : EntityBase
        {
            // Validate the input
            if (id == Guid.Empty)
            {
            ...
            // Auto-generate a partition key if one isn't provided.
            if (string.IsNullOrWhiteSpace(partitionKey))
            {
                partitionKey = Partition.Calculate(id);
            }
        ...
        /// <returns></returns>
        public async Task<T?> GetAsync<T>(Guid id, string? partitionKey = null, bool includedDeletedRecords = false) where T : EntityBase
        {
            // Validate the input
            if (id == Guid.Empty)
            {
...
#if DEBUG
            System.Diagnostics.Debug.WriteLine("Cosmos DB Query:" + debugQuery);
#endif
